...
function toprule(col_name::Array{String})
...
function bottomrule(col_name::Array{String})
  new_rule = map(x -> Base.similar(append_both_ends(x, ""), '-'), col_name)
  push!(new_rule, "") 
  @printf("%s\n", foldl((x, y) -> x * "+" *y, new_rule, init=""))
  # @printf("%s\n", new_rule2)
...
function header_column(names::Array{String})
...
function table_header(col_names::Array{String})
  # copy_col_names = map(x -> ensure_width(7, x), col_names)
  copy_col_names = col_names
  toprule(copy_col_names)
  header_column(copy_col_names)
  bottomrule(copy_col_names)
end


function add_row(header::Array{String}; data::Array{String})
